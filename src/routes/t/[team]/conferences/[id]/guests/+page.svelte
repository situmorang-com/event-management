        ...
        <tr class="border-t border-slate-100">
          ...
          <td class="px-4 py-2">
            <div class="flex items-center gap-1">
              <!-- Email -->
              {#if g.email}
                <a href={`mailto:${g.email}`} title={g.email} class="rounded p-1.5 text-slate-400 hover:bg-slate-100 hover:text-slate-700">
              ...
              {/if}
              <!-- Ticket -->
              <a href={`/ticket/${g.ticketCode}`} target="_blank" title="View ticket" class="rounded p-1.5 text-slate-400 hover:bg-slate-100 hover:text-slate-700">
              ...
              {#if g.whatsapp}
                ...
                {#if link}
                  <a href={link} target="_blank" rel="noopener noreferrer" title={g.whatsapp} class="rounded p-1.5 text-green-600 hover:bg-green-50 hover:text-green-700">
              ...
              <!-- Edit -->
              {#if data.role !== 'staff'}
                <button title="Edit guest" onclick={() => { editingId = g.id; }} class="rounded p-1.5 text-slate-400 hover:bg-slate-100 hover:text-slate-700">
                  <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125"/></svg>
                </button>
              {/if}
              <!-- Cancel -->
              {#if data.role !== 'staff' && g.status !== 'cancelled'}
                <form method="POST" action="?/cancel" class="inline">
                  ...
                  <button title="Cancel registration" class="rounded p-1.5 text-slate-400 hover:bg-red-50 hover:text-red-600">
